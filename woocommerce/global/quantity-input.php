...
?>
<!--<div class="quantity loading-dots-quantity">-->
<div class="quantity" data-cart_item_key="<?php echo esc_attr($cart_item_key); ?>">
    ...
    <div class="rknm-btn rknm-plus">
        <svg xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" class="plus_svg"
            style="width: 18px; height: 18px;" width="24" height="24" fill="currentColor">
            <defs>
                <symbol xmlns="http://www.w3.org/2000/svg" id="addSimple" viewBox="0 0 24 24">
                    <path fill-rule="evenodd" d="M13 4h-2v7H4v2h7v7h2v-7h7v-2h-7V4z" clip-rule="evenodd" />
                </symbol>
            </defs>
            <use xlink:href="#addSimple" />
        </svg>
    </div> <!-- $quantity + -->
    <div class="quantity_no">
        <input type="<?php echo esc_attr($type); ?>" <?php echo $readonly ? 'readonly="readonly"' : ''; ?>
            ...
            min="<?php echo esc_attr($min_value); ?>" <?php if (0 < $max_value): ?>
                max="<?php echo esc_attr($max_value); ?>" <?php endif; ?> <?php if (!$readonly): ?>
        ...
        <div class="loading-dots rknm-hidden" id="loading-dots">
            <div></div>
            <div></div>
            <div></div>
        </div>
        <?php if ($input_value == $max_value): ?>
            <span class="quantity_max" id="quantity_max">حداکثر</span>
        <?php else: ?>
            <span class="quantity_max rknm-hidden" id="quantity_max">حداکثر</span>
        <?php endif; ?>
    </div> <!-- $quantity input -->

    <div class="rknm-btn rknm-minus">
        <?php if ($input_value > $min_value/*1*/): ?>
        ...
        <?php else: ?>
            <svg clase="rknm-cart-remove-btn" id="rknm-cart-del-btn" width="16" height="16" viewBox="0 0 24 24"
