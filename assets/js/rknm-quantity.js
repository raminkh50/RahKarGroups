jQuery(document).ready(function ($) {
        ...
        if (response.success) {
          ...
          if ($("body").hasClass("single-product")) {
            if (response.data.item_removed) {
              $quantityWrapper.closest(".quantity_wrapper").hide();
  ...
  $(document).on("click", ".rknm-plus", function () {
    ...
    if (currentVal < max) {
      ...
      $input.val(newVal); // Update visually immediately
      if (cartItemKey) {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          $quantityInput.attr("data-cart_item_key", response.data.cart_item_key); // Use .attr() to be safe
        } else {
          toastr.error(response.data.message, "", {
            toastClass: "rknm-cart-toast-error",
            toastId: "rknm_cart_toast-" + Date.now(),
