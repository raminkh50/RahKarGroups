jQuery(document).ready(function ($) {
  ...
  $(document).on("click", ".single_add_to_cart_button", function (e) {
    ...
    var productId = $form.find("[name=product_id]").val() || $btn.val();
  ...
  $(".variations_form").on("show_variation", function (event, variation) {
    ...
    var $quantityInput = $quantityWrapper.find(".quantity");
    var $input = $quantityWrapper.find(".input-text");

    $.ajax({
        url: rknm_ajax_cart.ajax_url,
        type: 'POST',
        data: {
            action: 'rknm_check_variation_in_cart',
        ...
        success: function(response) {
            if (response.success && response.data.in_cart) {
                $quantityWrapper.removeClass("rknm-hidden");
                $button.addClass("rknm-hidden");
                $input.val(response.data.quantity);
                $quantityInput.attr("data-cart_item_key", response.data.cart_item_key);
            } else {
                $quantityWrapper.addClass("rknm-hidden");
                $button.removeClass("rknm-hidden");
            }
        }
    });
  });
